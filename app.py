...
area=st.slider("Area", min_value=0, max_value=1000, value=None, step=1)
...
furnish = st.selectbox('Condition of Flat',('Furnished','Semi-furnished','Unfurnished'))
...
if btn:
#     st.text(area)
#     st.text(bathroom_num)
#     st.text(bedroom_num)
#     st.text(locarr)
#     st.text(furnishing_locarr)
    other=[]
    other.append(area)
    other.append(bathroom_num)
    other.append(bedroom_num)
    other=np.array(other)
    pred=np.concatenate((other,locarr,furnishing_locarr))
    pred=list(pred)
#     st.text(pred)
